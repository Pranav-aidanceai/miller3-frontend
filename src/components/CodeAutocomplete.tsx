...
// Plain client-side API wrappers, not server actions — safe to reuse outside
// the search module.
import { filterResultsToOptions, getFilterOptionsAction, type FilterOption } from '@/app/(modules)/search/searchServices';
import { lettersOnly } from '@/app/(modules)/search/filterValidation';
...
/**
 * `sidebar` is the search filter panel's compact row; `form` matches the taller
 * inputs the admin forms use, and is the only variant that reacts to `invalid`.
 */
export type CodeAutocompleteVariant = 'sidebar' | 'form';
...
interface CodeAutocompleteProps {
    ...
    placeholder: string;
    variant?: CodeAutocompleteVariant;
    disabled?: boolean;
    invalid?: boolean;
    /** Extra content after the label text, e.g. a required marker. */
    labelSuffix?: React.ReactNode;
}

/**
 * Type-ahead picker over one of the code filter endpoints. The value is only
 * ever a code the API returned — free text is a query, never a selection.
 */
export const CodeAutocomplete = ({
    ...
    const isOpen = query.trim().length > 0 && !dismissed && !disabled;
                        ...
                        {value && (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className="shrink-0 cursor-pointer text-primary/60 transition-colors hover:text-primary disabled:cursor-not-allowed"
                                >
                                    <X className="h-3 w-3" />
                                </button>
                            </span>
