...
export async function GET(request: Request) {
    try {
        ...
        const response = await AXIOS.get(`/api/v1/admin/list_company_record_update_requests`, {
            params: {
                status,
                page,
                limit
...
export async function POST(request: Request) {
    try {
        ...
        const response = await AXIOS.post(`/api/v1/admin/review_company_record_update_request`, payload);
        return NextResponse.json(response.data, {
            status: response.status || 200
        });
    } catch (error: unknown) {
        console.error(
            'Error reviewing company update request:',
