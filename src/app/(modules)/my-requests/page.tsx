...
function RequestCard({ request }: { request: CompanyUpdateRequest }) {
    const [expanded, setExpanded] = useState(false);
    const changes = Object.entries(request.submitted_changes ?? {});
        ...
        <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-muted-foreground">
                        ...
                        {reviewed && ` · Reviewed ${formatTimestamp(request.reviewed_at!)}`}
                    </p>
                </div>
                <StatusBadge status={request.status} />
            </div>

            {/* Proposed fields, each carrying its own verdict once reviewed. */}
            ...
            {!reviewed && (
                <p className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Clock className="h-3.5 w-3.5" /> Waiting on an admin to review this request.
                </p>
            )}
        </div>
    );
...
export default function MyRequestsPage() {
    const [requests, setRequests] = useState<CompanyUpdateRequest[]>([]);
    const [status, setStatus] = useState<CompanyUpdateStatus | 'all'>('all');
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            setError(null);
            try {
                const response = await axios.get<CompanyUpdateRequestsResponse>('/api/company-update', {
                ...
                if (!active) return;
                setError(getErrorMessage(err, 'Failed to load your update requests'));
            } finally {
        ...
        <div className="flex flex-col p-5" style={{ height: 'calc(100vh - 3rem)' }}>
            <div className="shrink-0">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                    Company detail changes you&apos;ve proposed, newest first, and what the reviewer decided.
                </p>
                ...
                {!loading && !error && requests.length === 0 && (
                    <div className="mt-12 text-center text-muted-foreground">
                        <p className="text-lg font-medium">
                            {status === 'all' ? 'No requests yet' : `No ${STATUS_FILTERS.find(f => f.value === status)?.label.toLowerCase()} requests`}
                        </p>
                        <p className="mt-1 text-sm">
                            Open a company and use <span className="font-medium">Edit details</span> to propose a change.
                        </p>
                    </div>
                )}
