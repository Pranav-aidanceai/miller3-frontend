...
// ── Helpers ───────────────────────────────────────────────────────────────────
// Registration is a two-step wizard: step 0 collects name+email (gated by a
// "Next" button), step 1 collects password+confirm+Terms (gated by "Sign Up").

/** Fills step 0 and advances to step 1. */
const goToPasswordStep = async (overrides: Record<string, string> = {}) => {
  const values = { name: 'John Doe', email: 'john@example.com', ...overrides }
...
/** Drives the whole form to a submittable state, including accepting the
 * Terms of Use via its modal (the checkbox itself only opens that modal —
 * see TermsOfUse.tsx — the field is set by its "I accept" button). */
const fillForm = async (overrides: Record<string, string> = {}) => {
  ...
  await goToPasswordStep(overrides)
...
describe('RegisterPage', () => {
  ...
  describe('Step 0 - name & email', () => {
    ...
    it('does not render the password fields yet', () => {
      render(<RegisterPage />)
    ...
    it('renders the Next button as disabled initially', () => {
      render(<RegisterPage />)
    ...
    it('enables Next once name and email are valid, and advances to the password step', async () => {
      render(<RegisterPage />)
  ...
  // 2. Step 1 - password & terms
  describe('Step 1 - password & terms', () => {
    it('renders the password fields and checkbox', async () => {
      render(<RegisterPage />)
      await goToPasswordStep()
      expect(screen.getByPlaceholderText(/enter your password/i)).toBeInTheDocument()
    ...
    it('renders the Sign Up button as disabled initially', async () => {
      render(<RegisterPage />)
      await goToPasswordStep()
    ...
    it('enables Sign Up once the form is fully valid', async () => {
      render(<RegisterPage />)
      await fillForm()
      await waitFor(() => {
    ...
    it('returns to step 0 via Back, keeping the entered name/email', async () => {
      render(<RegisterPage />)
      await goToPasswordStep({ name: 'Jane Roe', email: 'jane@example.com' })
      await userEvent.click(screen.getByRole('button', { name: /back/i }))
      expect(screen.getByPlaceholderText(/enter your name/i)).toHaveValue('Jane Roe')
      expect(screen.getByPlaceholderText(/enter your email/i)).toHaveValue('jane@example.com')
    })
